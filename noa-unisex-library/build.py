...
IMG_DIR = "images"          # relative to index.html
IMG_EXT = "webp"            # preferred preview format (png/jpg also work)
...
def resolve_defaults(text):
    """Replace every {argument name="x" default="y"} with its default."""
    return re.sub(r'\{argument name="([^"]+)" default="([^"]*)"\}',
                  lambda m: m.group(2), text.replace('\\"', '"'))


def to_sentence(text):
    """Flatten a JSON prompt into one natural-language line (prose-model ready)."""
    try:
        obj = json.loads(text)
    except Exception:
...
def main():
    ...
    for i, p in enumerate(PROMPTS):
        ...
        image = f"{IMG_DIR}/{i + 1:02d}-{slugify(p['title'])}.{IMG_EXT}"
        records.append({
            "id": i,
            "category": p["category"],
            "title": p["title"],
    ...
    # ── Image manifest: one preview image per card, with generation prompts ──
    base = os.path.dirname(__file__)
    manifest = []
    for r in records:
        resolved = resolve_defaults(r["prompt"])
        manifest.append({
            "id": r["id"],
            "file": r["image"],
            "title": r["title"],
            ...
            "aspect": r["aspect"],
            "format": r["format"],
            "alt": f"{r['title']} — Noa Unisex preview ({r['category']})",
            "prompt": resolved,                 # as-authored, defaults filled
            "prompt_sentence": to_sentence(resolved),  # prose-model friendly
        })
    with open(os.path.join(base, "image-manifest.json"), "w", encoding="utf-8") as f:
        json.dump({"count": len(manifest), "dir": IMG_DIR, "ext": IMG_EXT,
                   "images": manifest}, f, ensure_ascii=False, indent=2)
    print(f"Wrote image-manifest.json ({len(manifest)} images)")

    # ── Human-readable checklist (IMAGES.md) ──
    lines = ["# Noa Unisex — preview image map\n",
             f"{len(manifest)} preview images, one per card. Generate each with the "
             "prompt below (any model), save it to the exact `file` path, and the "
             "card will show it automatically.\n",
             "| # | Save as | Card | Aspect | Done |",
             "|---|---------|------|--------|------|"]
    for m in manifest:
        lines.append(f"| {m['id']+1:02d} | `{m['file']}` | {m['title']} "
                     f"({m['category']}) | {m['aspect']} | ☐ |")
    lines.append("\n---\n\n## Generation prompts\n")
    for m in manifest:
        lines += [f"### {m['id']+1:02d}. {m['title']}",
                  f"- **Save as:** `{m['file']}`  ·  **Aspect:** {m['aspect']}  ·  **Format:** {m['format']}",
                  "\n**Prompt (structured / as authored):**\n",
                  "```\n" + m["prompt"] + "\n```",
                  "\n**Prompt (one-sentence, for Midjourney / DALL·E):**\n",
                  "```\n" + m["prompt_sentence"] + "\n```\n"]
    with open(os.path.join(base, "IMAGES.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")
    print("Wrote IMAGES.md")
